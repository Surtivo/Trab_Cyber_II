...
def detect_ip_lookup_dns(src, domain):
    ...
    if any(k in tokens for k in ["check", "whoami", "geo"]):
        score += 2

    if domain.endswith((".io", ".net", ".org")):
        score += 1

    if score >= 3:
        log(f"{timestamp()} | ALERT | Possível IP lookup (DNS): {domain}")
        add_score(src, score, "ip_lookup")
...
def handle_dns(pkt):
    if pkt.haslayer(DNSQR) and pkt.haslayer(IP) and pkt.haslayer(UDP):
        ...
        detect_ip_lookup_dns(pkt[IP].src, domain)
...
def detect_payload_download(flow, headers):
    lines = headers.split("\r\n")
    ...
    # Detecta GET suspeito
    if request_line.startswith("GET"):
        path = request_line.split(" ")[1]

        suspicious_ext = [".exe", ".bin", ".sh", ".elf", ".py", ".nips"]
        suspicious_keywords = ["payload", "download", "fetch", "update"]

        if any(ext in path for ext in suspicious_ext) or \
           any(k in path.lower() for k in suspicious_keywords):

            log(f"{timestamp()} | ALERT | Possível download de payload: {path}")
            add_score(flow[0], 8, "payload_download")

    # Detecta uso de ferramentas automáticas
    for line in lines:
        if "User-Agent" in line:
            if "wget" in line.lower() or "curl" in line.lower():
                log(f"{timestamp()} | ALERT | Ferramenta suspeita detectada: {line}")
                add_score(flow[0], 4, "suspicious_user_agent")

# ---------------- DETECT IP LOOKUP VIA HTTP ----------------
def detect_ip_lookup_http(flow, headers):
    lines = headers.split("\r\n")

    if not lines:
        return

    request_line = lines[0].lower()

    score = 0

    # path
    if "/ip" in request_line:
        score += 3

    if "json" in request_line:
        score += 1

    if any(k in request_line for k in ["whoami", "check", "geo"]):
        score += 2
    ...
    if score >= 4:
        log(f"{timestamp()} | ALERT | IP lookup detectado via HTTP")
        add_score(flow[0], score, "ip_lookup_http")
...
def handle_http(pkt):
    ...
    if "\r\n\r\n" in decoded:
        ...
        detect_payload_download(flow, headers)
        detect_ip_lookup_http(flow, headers)
...
def detect_ip_lookup(flow, headers):
    lines = headers.split("\r\n")
    ...
    path_flag = is_ip_lookup_path(path)
    agent_flag = is_automated_agent(headers)

    if path_flag:
        log(f"{timestamp()} | ALERT | IP lookup via path: {path}")
        add_score(flow[0], 5, "ip_lookup_path")

    if path_flag and agent_flag:
        log(f"{timestamp()} | ALERT | IP lookup automatizado (forte)")
        add_score(flow[0], 8, "ip_lookup_strong")